...
public class ControlloreVisUtenti implements Initializable, Archiviabile<Utente, UtenteInvalidoException> {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        filteredData = new FilteredList<>(archivioUtenti, p -> true);
    ...
    private void aggiornaNomeUtenti(TableColumn.CellEditEvent<Utente, String> event) {
        // modifica il campo nome dell'elemento all'interno dell'archivioUtenti,
        // corrispondente a quella riga nella tabella, con il testo inserito a mano per
        // la modifica
        try {
    ...
    private void aggiornaCognomeUtenti(TableColumn.CellEditEvent<Utente, String> event) {
        // modifica il campo cognome dell'elemento all'interno dell'archivioUtenti,
        // corrispondente a quella riga nella tabella, con il testo inserito a mano per
        // la modifica
        try {
    ...
    private void aggiornaMailUtenti(TableColumn.CellEditEvent<Utente, String> event) {
        // modifica il campo mail dell'elemento all'interno dell'archivioUtenti,
        // corrispondente a quella riga nella tabella, con il testo inserito a mano per
        // la modifica
        try {
    ...
    private void rimuoviUtente(ActionEvent event) {
        ...
        if (daEliminare == null) {
            Alert alert = new Alert(Alert.AlertType.WARNING, "Nessun utente selezionato.", ButtonType.OK);
            alert.showAndWait();
            return;
        }

        // verifica che non abbia prestiti attivi e rimuove l'utente
        if (cvp.inPrestitoAttivoUtente(daEliminare)) {
            Alert alert = new Alert(Alert.AlertType.ERROR, "Impossibile eliminare: l'utente ha prestiti attivi.",
                    ButtonType.OK);
            alert.showAndWait();
        } else {
    ...
    private void ricercaUtenti(KeyEvent event) {
        String filter = testoRicercaUtenti.getText();
        ...
        if (filter == null || filter.length() == 0) {
            ...
            filteredData.setPredicate(s -> {
                if (tipo.equals("Cognome")) {
                    return s.getCognome().toLowerCase().contains(filter.toLowerCase());
                } else if (tipo.equals("Matricola")) {
                    return s.getMatricola().toLowerCase().contains(filter.toLowerCase());
                }
                return false;
            });
        }
    ...
    private void selezionaUtente(MouseEvent event) {
        // carica l'utente selezionato nell'utentePrePrestito, altrimenti cattura
        // un'eccezione
        try {
    ...
    public void inserisciNuovoElemento(Utente nuovoElemento) throws UtenteDuplicatoException {
        ...
        if (isElementoPresente(nuovoElemento) == true)
